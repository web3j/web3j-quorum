...
class QuorumTransactionManager(
        ...
        val attempts: Int = DEFAULT_POLLING_ATTEMPTS_PER_TX_HASH
) : RawTransactionManager(web3j, credentials, attempts, sleepDuration) {
    ...
    override fun signAndSend(rawTransaction: RawTransaction): EthSendTransaction {
        ...
        if (privateFor.isNotEmpty()) {
            ...
            val privateTransaction = RawTransaction.createTransaction(
                    rawTransaction.nonce, rawTransaction.gasPrice,  rawTransaction.gasLimit,
                    rawTransaction.to, rawTransaction.value, responseDecoded)
