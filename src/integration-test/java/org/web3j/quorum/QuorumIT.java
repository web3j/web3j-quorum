 ...
 *
 * <p>To use, simply run: <code>git clone https://github.com/blk-io/crux.git</code> <code>
 * docker-compose -f docker/quorum-crux/docker-compose.yaml up</code>
 *
...
public class QuorumIT {
    ...
    private static final Node quorum1 =
            new Node(
    ...
    private void runPrivateHumanStandardTokenTest(Node sourceNode, Node destNode) throws Exception {
        ...
        TransactionReceipt aliceTransferReceipt =
                contract.transfer(destNode.getAddress(), transferQuantity).send();
        ...
        TransactionReceipt approveReceipt =
                contract.approve(destNode.getAddress(), transferQuantity).send();
